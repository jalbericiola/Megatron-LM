...
FAMILY_RELATIVE_L2_TOLERANCE = 0.18
FAMILY_COSINE_TOLERANCE = 0.97
LOCALIZED_RELATIVE_L2_TOLERANCE = 0.03
LOCALIZED_COSINE_TOLERANCE = 0.995
...
def _assert_gradient_parity(reference, candidate):
    ...
    for name in reference:
        ...
        families.setdefault(family, []).append(
            (name, reference[name].flatten(), candidate[name].flatten())
        )

    assert families.keys() == {"attention", "moe", "projection_and_norm"}
    for family, parts in families.items():
        reference_parts = [reference_part for _name, reference_part, _candidate_part in parts]
        candidate_parts = [candidate_part for _name, _reference_part, candidate_part in parts]
        ...
        assert (
            relative_l2 < FAMILY_RELATIVE_L2_TOLERANCE
        ), f"{family}: relative L2 {relative_l2} >= {FAMILY_RELATIVE_L2_TOLERANCE}"
        assert (
            cosine > FAMILY_COSINE_TOLERANCE
        ), f"{family}: cosine {cosine} <= {FAMILY_COSINE_TOLERANCE}"

        # Family concatenation can hide a localized defect in one parameter behind
        # a much larger sibling tensor. Check every numerically active parameter at
        # the same tolerance; only tensors below 1e-4 of the family norm are treated
        # as inactive to avoid making near-zero BF16 noise a parity oracle.
        active_floor = reference_norm * 1.0e-4
        for name, reference_part, candidate_part in parts:
            reference_part = reference_part.double()
            candidate_part = candidate_part.double()
            assert torch.isfinite(reference_part).all(), f"{name}: non-finite dense gradient"
            assert torch.isfinite(candidate_part).all(), f"{name}: non-finite shared gradient"
            reference_part_norm = reference_part.norm()
            candidate_part_norm = candidate_part.norm()
            if reference_part_norm <= active_floor and candidate_part_norm <= active_floor:
                continue
            assert (
                reference_part_norm > active_floor
            ), f"{name}: localized dense gradient is inactive while the shared gradient is not"
            assert (
                candidate_part_norm > active_floor
            ), f"{name}: localized shared gradient is inactive while the dense gradient is not"
            part_relative_l2 = (
                (reference_part - candidate_part).norm() / reference_part_norm
            ).item()
            part_cosine = torch.nn.functional.cosine_similarity(
                reference_part.unsqueeze(0), candidate_part.unsqueeze(0)
            ).item()
            assert part_relative_l2 < LOCALIZED_RELATIVE_L2_TOLERANCE, (
                f"{name}: localized relative L2 {part_relative_l2} "
                f">= {LOCALIZED_RELATIVE_L2_TOLERANCE}"
            )
            assert (
                part_cosine > LOCALIZED_COSINE_TOLERANCE
            ), f"{name}: localized cosine {part_cosine} <= {LOCALIZED_COSINE_TOLERANCE}"


def test_gradient_parity_rejects_defect_diluted_by_large_family_tensor():
    reference = {
        "mtp.layers.0.mtp_model_layer.layers.0.large.weight": torch.ones(10_000),
        "mtp.layers.0.mtp_model_layer.layers.0.localized.weight": torch.ones(1),
        "mtp.layers.0.mtp_model_layer.layers.1.weight": torch.ones(4),
        "mtp.layers.0.norm.weight": torch.ones(4),
    }
    candidate = {name: gradient.clone() for name, gradient in reference.items()}
    _assert_gradient_parity(reference, candidate)
    # A 4% error is hidden by the 10,000-element sibling and passes the old
    # family/localized 0.18 tolerance, but must fail the 0.03 localized oracle.
    candidate["mtp.layers.0.mtp_model_layer.layers.0.localized.weight"].mul_(0.96)

    with pytest.raises(AssertionError, match="localized.*localized relative L2"):
        _assert_gradient_parity(reference, candidate)
