...
def loss_func(loss_mask, kl_term, ratios, entropy_term, truncated_from_above, truncated_from_below, output_tensor):
    args = get_args()
    ...
    if args.check_for_nan_in_loss_and_grad:
        rerun_state_machine.validate_result(result=loss[0], rejection_func=torch.isnan, message="found NaN in local forward loss calculation", tolerance=0.0, fatal=True)
    if args.check_for_spiky_loss:
        rerun_state_machine.validate_result(result=loss[0], rejection_func=partial(rerun_state_machine.is_spiky_loss, threshold=SPIKY_LOSS_PERC), message="Spiky loss", tolerance=0.0, fatal=False)
    ...
    reporting_truncated_from_above = torch.cat([masked_truncated_from_above.clone().detach().view(1), total_tokens.view(1)])
    reporting_truncated_from_below = torch.cat([masked_truncated_from_below.clone().detach().view(1), total_tokens.view(1)])
...
def forward_step(data_iterator, model: GPTModel, loss_only: bool = False):
    ...
    if args.rl_use_sequence_packing:
        ...
        (tokens, advantages, old_logprobs, loss_mask, position_ids, ref_logprobs, inference_logprobs, seq_starts, seq_lengths, seq_indices, packed_seq_params) = load_packed_data_by_index(bin_tensor.item(), runtime_state.packing_context, args.rl_inference_logprobs_is_correction)
        runtime_state.increment_sequences(len(seq_indices))
    else:
        (tokens, advantages, old_logprobs, loss_mask, position_ids, ref_logprobs, inference_logprobs) = batch_data
        seq_starts = None
        seq_lengths = None
        ...
        inference_logprobs = inference_logprobs.cuda() if args.rl_inference_logprobs_is_correction else None
        runtime_state.increment_sequences(tokens.shape[0])

    model_to_use = model[0] if isinstance(model, list) else model
    if packed_seq_params is None:
        packed_seq_params = get_default_packed_seq_params(seq_length=tokens.shape[1], device=tokens.device)
    ...
    with stimer:
        with sol_nvtx_range("rl/train/forward"):
            logprobs_or_hidden_states = get_logprobs(model_to_use, tokens, position_ids, no_grad=False, packed_seq_params=packed_seq_params)
        ...
        if not is_pipeline_last_stage():
            ...
            with sol_nvtx_range("rl/train/grpo-loss"):
                loss, kl_term, ratios, entropy_term, truncated_from_above, truncated_from_below = calculate_grpo_loss(
                    current_logprobs=current_logprobs, old_logprobs=old_logprobs, ref_logprobs=ref_logprobs,
                    advantages=advantages, clamp_eps_lower=args.grpo_clamp_eps_lower, clamp_eps_upper=args.grpo_clamp_eps_upper,
                    kl_beta=args.grpo_kl_beta, entropy_weight=args.grpo_entropy_term_weight, inference_logprobs=inference_logprobs,
                    is_truncation_coef=args.rl_importance_sampling_truncation_coef, seq_starts=seq_starts, seq_lengths=seq_lengths)
            output_tensor = loss

    return output_tensor, partial(loss_func, loss_mask, kl_term, ratios, entropy_term, truncated_from_above, truncated_from_below)
...
def train_valid_test_datasets_provider(train_val_test_num_samples):
    ...
    class MinimalDataset:
        ...
        def __getitem__(self, idx):
            return {'text': torch.ones(1, dtype=torch.long), 'tokens': torch.ones(1, dtype=torch.long), 'labels': torch.ones(1, dtype=torch.long), 'position_ids': torch.zeros(1, dtype=torch.long), 'attention_mask': torch.ones(1, dtype=torch.bool), 'loss_mask': torch.ones(1, dtype=torch.float)}

    train_ds = MinimalDataset(size=((args.global_batch_size * args.train_iters) if args.train_iters else args.train_samples))
    valid_ds = MinimalDataset(size=(args.eval_iters * args.global_batch_size) * ((args.train_iters if args.train_iters else (args.train_samples // args.global_batch_size)) // args.eval_interval))
...
if __name__ == "__main__":
    ...
    def _model_builder(args, pre_process, post_process, vp_stage=None, config=None, pg_collection=None):
        if getattr(args, "is_hybrid_model", False):
            return mamba_builder(args, pre_process, post_process, vp_stage, config=config, pg_collection=pg_collection)
        else:
            return _gpt_builder(args, pre_process, post_process, vp_stage, config=config, pg_collection=pg_collection)

    pretrain(None, partial(model_provider, _model_builder), ModelType.encoder_or_decoder, forward_step, args_defaults={})
