...
class MambaMixer(MegatronModule):
    ...
    def post_refit(self) -> None:
        ...
        The separate RL inference model never enters train mode, so without this
        hook the lazy stale-flag path never fires and generation keeps the A matrix
        from whenever the cache was last filled.
        """
        if getattr(self, "_A_neg_exp_cache", None) is not None:
