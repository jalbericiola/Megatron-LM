    ...
    def forward(
        ...
        *,
        ...
        if sp_cp_ctx is not None:
            ...
            # per-segment-zigzag shard (post-RoPE). Gather Q/K/V to global order, run the exact
            # two-term flash composition (prefix cross-attention + branch-causal attention), then
            # select this rank's query rows. This avoids FlexAttention create_block_mask's O(T^2)
            # temporary, which exceeds 60 GiB near the configured 49k sequence limit.
            from megatron.core.models.hybrid.shared_prefix import (
                local_tree_attention,
                two_term_tree_attention,
            )

            layout, cp_group = sp_cp_ctx
            key_full, value_full = layout.gather_kv(key, value, cp_group)
            if query.dtype in (torch.float16, torch.bfloat16):
                query_full = layout._gather(query, cp_group)
                core_attn_full = two_term_tree_attention(
                    query_full,
                    key_full,
                    value_full,
                    layout.global_seg_lens[0],
                    layout.global_seg_lens[1:],
                    real_prefix_len=layout.real_seg_lens[0],
                    real_completion_lens=layout.real_seg_lens[1:],
                )
                core_attn_out = core_attn_full.index_select(0, layout.local_global_pos)
            else:
                # FlashAttention does not support FP32. Keep FlexAttention as a correctness
                # fallback for small numerical tests; production BF16 always takes the O(T) path.
                core_attn_out = local_tree_attention(
                    query, key_full, value_full, layout.local_block_mask()
                )
